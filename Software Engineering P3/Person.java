...
public class Person {
  ...
  public void setName() {
    ...
    while (name.isEmpty()) {
      System.out.println("Invalid enty. Please retry");
      name = sc.nextLine();
    }
    name.strip();
    this.name = name;
  ...
  public void setTelNumber() {
    //ensure use inputs a valid telephone number
    System.out.println("Enter telephone number");
    String telNumber = sc.nextLine();
    while (((telNumber.startsWith("0")) & (telNumber.length() >= 10) & (telNumber.length() < 13)) == false) {
      System.out.println("Please enter a valid telephone number that start with with 0)");
