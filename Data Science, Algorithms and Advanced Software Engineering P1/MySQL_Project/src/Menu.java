 ...
 * It uses a JDBC driver to access external information from the database 'Poise_PMS' for use within the program. 
 * A menu with options is displayed to the user, which enables them to add new projects, edit existing projects etc., 
 * the results of which are then displayed to the user, and updated in the external database. The 'Poise_PMS' database 
		...
		while(true) {
			...
			try {
				Connection connection = DriverManager.getConnection(
                    "jdbc:mysql://localhost:3306/Poise?useSSL=false",
                    "otheruser",
                    "swordfish"
                    );
